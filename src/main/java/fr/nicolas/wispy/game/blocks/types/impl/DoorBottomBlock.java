...
import fr.nicolas.wispy.game.blocks.types.PairedBottomBlock;
...
public class DoorBottomBlock extends PairedBottomBlock {

    protected boolean open = false;

    private final BufferedImage sideTexture = Assets.get("blocks/door_side");

    public DoorBottomBlock() {
        super(Blocks.DOOR_TOP, true);
        type(Blocks.DOOR_BOTTOM);
    ...
    public void onRightClick(WorldManager worldManager, int x, int y) {
        ...
        if (block.getType() == pairedTopBlock) {
            DoorTopBlock doorBottomBlock = (DoorTopBlock) block;
